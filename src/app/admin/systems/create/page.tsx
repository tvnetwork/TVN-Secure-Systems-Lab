...
export default function CreateSystemPage() {
  ...
  const [layerInputs, setLayerInputs] = useState({
    perimeter: "", detection: "", access: "", response: "", asset: ""
  });
  ...
  const addLayerItem = (layerType: keyof typeof layerInputs) => {
    ...
    if (item) {
      setFormData(prev => ({
        ...
        layers: {
          ...prev.layers,
          [layerType]: [...prev.layers[layerType], item]
        }
      }));
  ...
  const removeLayerItem = (layerType: keyof typeof layerInputs, index: number) => {
    setFormData(prev => ({
      ...
      layers: {
        ...prev.layers,
        [layerType]: prev.layers[layerType].filter((_, i) => i !== index)
      }
    }));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('systems').insert([formData]).select();
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 pb-12">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Create New System</h1>
          <p className="text-muted-foreground mt-1">Configure security layers and properties.</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="p-6 rounded-2xl border border-white/10 bg-black/40 backdrop-blur-md space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <input required type="text" value={formData.name} onChange={handleNameChange} className="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="e.g. Fort Knox" />
            </div>
            <div className="space-y-2">
              <label className="text-sm font-medium text-muted-foreground">URL Slug</label>
              <input required type="text" value={formData.slug} onChange={e => setFormData(prev => ({ ...prev, slug: e.target.value }))} className="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="fort-knox" />
            </div>
            ...
            <div className="md:col-span-2 space-y-2">
              ...
              <textarea required rows={3} value={formData.description} onChange={e => setFormData(prev => ({ ...prev, description: e.target.value }))} className="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Short description of the system..." />
            </div>
        ...
        <div className="p-6 rounded-2xl border border-white/10 bg-black/40 backdrop-blur-md space-y-6">
          <h2 className="text-xl font-semibold border-b border-white/10 pb-4">Security Layers (JSON Construction)</h2>
          <p className="text-sm text-muted-foreground mb-4">Add defense components for each layer type. Press enter or click Add.</p>
            ...
            {(Object.keys(formData.layers) as Array<keyof typeof formData.layers>).map((layer) => (
              <div key={layer} className="p-4 rounded-xl border border-white/5 bg-white/5">
                ...
                <div className="flex gap-2 mb-3">
                  <input
                    type="text"
                    value={layerInputs[layer]}
                    onChange={e => setLayerInputs(prev => ({ ...prev, [layer]: e.target.value }))}
                    onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); addLayerItem(layer); } }}
                    ...
                    placeholder={`Add item to ${layer}...`}
                  />
                  <button type="button" onClick={() => addLayerItem(layer)} className="px-4 py-2 bg-white/10 hover:bg-white/20 rounded-lg text-sm transition-colors">Add</button>
                </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {formData.layers[layer].map((item, idx) => (
                    <span key={idx} className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-500/20 text-blue-200 text-xs border border-blue-500/30">
                      {item}
                      <button type="button" onClick={() => removeLayerItem(layer, idx)} className="hover:text-red-400">&times;</button>
                    </span>
                  ))}
        ...
        <div className="flex justify-end gap-4">
          ...
          <button type="submit" disabled={loading} className="flex items-center gap-2 px-8 py-3 rounded-xl bg-blue-600 text-white font-medium hover:bg-blue-700 transition-colors disabled:opacity-50">
            {loading ? "Saving..." : <><Save className="w-5 h-5" /> Create System</>}
          </button>
        </div>
      </form>
    </div>
